...
import { Phone, PhoneCall, MapPin, AtSign, Copy, CheckCircle2, XCircle } from "lucide-react";
...
export default async function OrderPage({ params }: Props) {
  ...
  const isOwner = user?.role !== "ADMIN" && order.userId === user?.id;
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-10">
      <div className="text-center mb-8">
        <span
          className={`inline-grid size-14 place-items-center rounded-full mb-3 ${
            isCancelled
              ? "bg-destructive/10 text-destructive"
              : "bg-emerald-100 text-emerald-600"
          }`}
        >
          {isCancelled ? <XCircle className="size-7" /> : <CheckCircle2 className="size-7" />}
        </span>
        <h1 className="text-2xl font-bold text-navy">
          {isCancelled ? "این سفارش لغو شده است" : "سفارش شما ثبت شد"}
        </h1>
        <p className="text-sm text-muted-foreground mt-1">
          {isCancelled
            ? "در صورت تمایل می‌توانید دوباره از فروشگاه سفارش ثبت کنید."
